    ...
    <table class="table table-striped- table-bordered table-hover table-checkable  no-footer dtr-inlin kt-datatable" id="html_table" width="100%">
        ...
        <thead class="flip-content">
            <tr class="info">
                ...
                <th align="center" width="25px"><font size="-1"> Usuario</font></th>
                <th  align="center"><font size="-1"> Nombre Completo</font></th>
        ...
        <tbody>
            <?php if ($usuarios) { ?>
                <?php foreach ($usuarios as $lis) { ?>
                    <?php $nombre = ''; ?>
                   <?php $idTienda=0; ?>
                  <?php if ($lis->getTiendaId()) { ?>
                   <?php $idTienda=$lis->getTiendaId(); ?>
                  <?php } ?>
                    <?php  $sqlexp = 'select nombre from tienda where id =' . $idTienda ?>
                    <?Php  $con = Propel::getConnection(); ?>
                    <?Php  $stmt = $con->prepare($sqlexp); ?>
                    <?Php  $resource = $stmt->execute(); ?>
                    <?Php  $rResult = $stmt->fetchAll(PDO::FETCH_ASSOC); ?> 
                    <?Php  foreach ($rResult as $reg) { ?>
                        <?php  $nombre = $reg['nombre']; ?>
                    <?php   } ?>
