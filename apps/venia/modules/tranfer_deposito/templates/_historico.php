                    ...
                    <tbody>
                        <?php foreach ($operaciones as $regi) { ?>
                            ...
                            <tr>
                                ...
                                <td><font size="-2"><?php echo $regi->getMedioPago()->getNombre(); ?></font></td>
     <!--                        <td><?php echo $regi->getDocumento(); ?></td> -->
                                <td style="text-align: right" >
                                <font size="-2">    <a href="<?php echo url_for('tranfer_deposito/muestra?id=' . $regi->getId()) ?>" class="btn btn-sm btn-block btn-secondary btn-dark" >
                                        <?php echo Parametro::formato($regi->getValor()); ?>
                                    </a>
                                </td> </font>                    
                                <td>
                                    <font size="-2"><?php echo $regi->getObservaciones(); ?></font> </td> 
                                                  <td>
                                                      
                                                         <?php if ($regi->getFechaDocumento('Ym') == date('Ym')) { ?>
                                                      <a href="<?php echo url_for("proceso/rechaza?tipo=debito&token=" . $regi->getId()) ?>" class="btn btn-sm btn-block btn-danger" data-toggle="modal" data-target="#ajaxmodalRECHAZA<?php echo $regi->getId(); ?>"> <li class="fa flaticon2-cancel"></li>    </a>
                                                         <?php } ?>
                                                  </td>
