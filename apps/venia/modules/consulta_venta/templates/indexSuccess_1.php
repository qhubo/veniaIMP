...
<div class="kt-portlet kt-portlet--responsive-mobile">
    ...
    <div class="kt-portlet__body">
        ...
        <div class="row"  style="padding-bottom:3px;">
            ...
            <label class="col-lg-2 control-label right " style="font-weight:bold; text-align: right;"> Vendedor </label>
            <div class="col-lg-2">
        ...
        <table class="table table-bordered  dataTable table-condensed flip-content" >
          ...
          <?php         $valor = $lista->getValorTotal() ;  ?>
         <?php      if ($lista->getAnulado()) {  ?>
          <?php        $valor= $lista->getValorTotal();  ?>
               <?php  if ($lista->getFecha('Ymd')== $lista->getFechaAnulo('Ymd')) { ?>
                <?php        $valor= 0;  ?>
          <?php    }  ?>
          <?php    }  ?>
                  <?php $TOTAL_VALOR =$valor+$TOTAL_VALOR; ?>
                <?php $TOTAL_PAGADO = $lista->getValorPagado()+$TOTAL_PAGADO; ?>
                    <?php $id = $lista->getId(); ?>
                    ...
                    <tr>
                        ...
                        <td style="text-align:right"><?php echo Parametro::formato($valor) ?> </td>
                        <td>
                ...
                <?php } ?>
                     ...
                     <?php foreach ($operacionesNotas as $lista) { ?>
  <?php $TOTAL_VALOR = $TOTAL_VALOR+$lista->getValorNota(); ?>

                    <?php $id = $lista->getId(); ?>
                    <?php $val = explode('-', $lista->getFaceFirma()) ?>
                    <?php $numero = $val[0]; ?>
                    <tr style="background-color:#FAF9D4">
                        <td>
                            <a class="btn btn-warning" href="<?php echo url_for('reporte_venta/muestra?id=' . $id) ?>" data-toggle="modal" data-target="#ajaxmodal<?php echo $id ?>">  <?php echo $lista->getCodigo() ?> </a>
                        </td>
                        <td> <?php echo substr($lista->getTienda(), 0, 20) ?>  </td>
                        <td><?php echo $lista->getFechaAnulo('d/m/Y H:i') ?></td>
                        <td> <font size="-1"><?php echo $lista->getUsuario() ?></font>  </td>
                        <td><?php
                            if ($lista->getClienteId()) {
                                echo $lista->getCliente()->getCodigoCli();
                        ...
                        <td style="text-align:right">
                            <?php echo Parametro::formato($lista->getValorNota()) ?> 
                        </td>
                        <td>
                        <a target="_blank" href="<?php echo url_for('pdf/factura?tok=' . $lista->getCodigo()) ?>" class="btn btn-block btn-xs btn-info " target = "_blank">
                            <?php echo $numero ?>
            ...
            <tr>
                ...
                <th><?php echo Parametro::formato($TOTAL_VALOR) ?> </th>
                          <td colspan="4"> </td>
                             <th><?php echo Parametro::formato($TOTAL_PAGADO) ?> </th>
                                                       <td colspan="2"> </td>
