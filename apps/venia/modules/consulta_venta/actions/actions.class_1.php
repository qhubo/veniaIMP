...
class consultaddddddd_ventaActions extends sfActions {
    ...
    public function executeReporteExcel(sfWebRequest $request) {
        ...
        foreach ($operaciones as $lista) {
            ...
            $valor = $lista->getValorTotal() ;
             if ($lista->getAnulado()) {
                $valor= $lista->getValorTotal() ;
                      if ($lista->getFecha('Ymd')== $lista->getFechaAnulo('Ymd')) { 
                    $valor= 0;  
          }  
            }
            ...
            $datos[] = array("tipo" => 2, "valor" => $valor);
        ...
        $operacionesNotas = $this->datosNotas($valores);        
//        echo "<pre>";
//        print_r($operacionesNotas);
//        die();
        
                foreach ($operacionesNotas as $lista) {
            $fila++;
            $datos = null;
            $codigCli = '';
            if ($lista->getClienteId()) {
            ...
            $datos[] = array("tipo" => 3, "valor" => substr($lista->getTienda(), 0, 20));  // ENTERO
            $datos[] = array("tipo" => 3, "valor" => $lista->getCodigo());  // ENTERO
            $datos[] = array("tipo" => 3, "valor" => $lista->getFechaAnulo('d/m/Y H:i'));  // ENTERO
            $datos[] = array("tipo" => 3, "valor" => $lista->getUsuario());
            $datos[] = array("tipo" => 3, "valor" => $codigCli);
            $datos[] = array("tipo" => 3, "valor" => $lista->getNombre());
            $datos[] = array("tipo" => 3, "valor" => $lista->getNit());
            $datos[] = array("tipo" => 3, "valor" => $lista->getEstatus());
            $datos[] = array("tipo" => 2, "valor" => $lista->getValorNota());
            $datos[] = array("tipo" => 3, "valor" => $lista->getFaceFirma());
            $datos[] = array("tipo" => 3, "valor" => $vende);
            $datos[] = array("tipo" => 3, "valor" => $lista->getObservaciones());
            $datos[] = array("tipo" => 3, "valor" => $lista->getRutaCobro());
            $datos[] = array("tipo" => 2, "valor" => 0);
            $datos[] = array("tipo" => 3, "valor" => '');
            $datos[] = array("tipo" => 3, "valor" => '');
            $TOTALvENTA = $TOTALvENTA+ $lista->getValorNota();
...
//        die();
    ...
    public function executeIndex(sfWebRequest $request) {
        ...
        $this->operaciones = $this->datos($valores);
           $this->operacionesNotas = $this->datosNotas($valores);
    }
...
//        $empresaId = $usuarioQue->getEmpresaId();
    ...
    public function datosNotas($valores) {
        ...
        $empresq = EmpresaQuery::create()->findOne();
        $listab = TiendaQuery::TiendaActivas(); // ctivas();
        $notas = NotaCreditoQuery::create();
        $notas->where("NotaCredito.Fecha >= '" . $fechaInicio . " 00:00:00" . "'");
        $notas->where("NotaCredito.Fecha <= '" . $fechaFin . " 23:59:00" . "'");   
        $notas->find();
        ...
        $operaciones = OperacionQuery::create();
        $operaciones->where("Operacion.FechaAnulo >= '" . $fechaInicio . " 00:00:00" . "'");
        $operaciones->where("Operacion.FechaAnulo <= '" . $fechaFin . " 23:59:00" . "'");   
...
//       ECHO $tipoUsuario;
        if ($tipoUsuario == 'VENTA') {
            $operaciones->filterByVendedorId($usuarioQue->getVendedorId());
        }
         if ($valores['busqueda']) {
            $operaciones->where("Operacion.Nombre like  '%" . $valores['busqueda'] . "%'");
        }
        ...
        $operaciones->filterByEmpresaId($usuarioQue->getEmpresaId());
        $operaciones->orderByFechaAnulo('Asc');
        $operaciones = $operaciones->find();
        return $operaciones;
    }
