...
//            $this->redirect('orden_cotizacion/nueva?codigo=' . $ordenQ->getCodigo());
            ...
            $codigoList = "LIST-" . $ordenQ->getCodigo();
            $RENUEVA = OrdenCotizacionQuery::create()->findOneByCodigo($codigoList);
            if (!$RENUEVA) {
                $RENUEVA = new OrdenCotizacion;
                $RENUEVA->setCodigo($codigoList);
                $RENUEVA->save();
            }
            ...
            $RENUEVA->save();
            $RENUEVAiD =$RENUEVA->getId();
            $historial = OrdenCotizacionDetalleQuery::create()
                    ->filterByOrdenCotizacionId($RENUEVA->getId())
                    ->find();
            if ($historial) {
                $historial->delete();
            }
            ...
            $this->redirect('pedido_pendiente/index?id=' . $RENUEVAiD);
        } catch (Exception $e) {
            $con->rollBack();
         //   $this->getUser()->setFlash('error', 'Intenta nuevamente  '.$e->getMessage());
