...
<div class="kt-portlet__body">
    ...
    <div class="row mb-3">
        <div class="col-md-5">
            <div class="input-group">
                ...
                <input type="text"  id="buscarPedido"   class="form-control"  placeholder="Buscar pedido, cliente, NIT, usuario...">
            </div>
        </div>

        <div class="col-md-7 text-right">  <span id="contadorPedidos" class="text-muted"></span> </div>
    </div>
 ...
 <?php foreach ($detalles as $reg) { ?>
               ...
               <tr class="pedido-row">
                        ...
                        <td> <?php if ($reg->getOrdenCotizacion()->getClienteId()) {
                            echo $reg->getOrdenCotizacion()->getCliente()->getCodigoCli();
                        } ?> 
                            <BR>
    <?php echo $reg->getOrdenCotizacion()->getNombre(); ?></td>
                        ...
                        <td style="text-align: right; font-size: +2">
                            <a class="btn btn-block  btn-sm " data-toggle="modal" href="#staticPP<?php echo $reg->getId() ?>"> <?php echo $reg->getCantidadTotal(); ?> </a>
                        </td>
                        <td style="text-align: right; font-size: +2">
    <?php echo Parametro::formato($reg->getOrdenCotizacion()->getValorTotal()); ?>
                        ...
                        <td> 
                            <a target="_blank" href="<?php echo url_for('reporte/ordenCotizacion?token=' . $orden->getToken()) ?>" class="btn btn-sm btn-block btn-warning"> <i class="flaticon2-printer"></i></a>
...
$(document).ready(function () {
    var totalPedidos = $('#tablaPedidos tbody tr').length;
    function actualizarContador() {
        var visibles = $('#tablaPedidos tbody tr.pedido-row:visible').length;
        if ($('#buscarPedido').val().trim() === '') {
            $('#contadorPedidos').text('Total de pedidos: ' + totalPedidos);
        } else {
            $('#contadorPedidos').text(
                'Mostrando ' + visibles + ' de ' + totalPedidos + ' pedidos'
            );
        }
    }
    $('#buscarPedido').on('keyup', function () {
        var texto = $(this).val().toLowerCase().trim();
        $('#tablaPedidos tbody tr.pedido-row').each(function () {
