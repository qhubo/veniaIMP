        ...
        <tbody>
            ...
            <?php foreach ($listado as $registro) { ?>
                ...
                <tr <?php if (($can % 2) == 0) { ?>  style="background-color:#ebedf2"  <?php } ?> >
                    ...
                    <td><?php echo substr($registro->getProducto()->getCodigoSku(), -6); ?>
                      </td>    
                    <td><?php echo $registro->getProducto()->getNombre(); ?></td>    
                    <td><?php //echo $registro->getValorUnitario();   ?>
                        <input  disabled=""  class="form-control " value="<?php echo $val ?>"  step="any" id="consulta_valor_<?php echo $lista->getId() ?>"  
                    ...
                    <td><?php //echo $registro->getCantidad();   ?>
                        <input min="1"  type="number"  class="form-control xlarge cantidad" value="<?php echo $can ?>"  id="consulta_numero_<?php echo $pid ?>"  
                           max="<?php  echo  $lista->getProducto()->getExistencia(); ?>"    name="consulta[numero_<?php echo $pid ?>]" onkeypress='validate<?php echo $pid ?>(event)' >
                    </td>    
                ...
                <tr  <?php if (($can % 2) == 0) { ?>  style="background-color:#ebedf2"  <?php } ?> >
                    ...
                    <td colspan="4">
                        <textarea name="consulta[det_<?php echo $lista->getId() ?>]" id="consulta_det_<?php echo $lista->getId() ?>"  name="textarea" rows="2" class="form-control" ><?php echo $detalle ?></textarea>
  </td>
